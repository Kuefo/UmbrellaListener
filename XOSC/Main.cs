...
namespace API {
    class Terminal {
        ...
        private static void Initialize() {
            API Server = new API();
            Server.Start();
            Console.Title = "XBLUmbrella XOSC API";
    ...
    class API {
        ...
        private TcpListener TCPListener = new TcpListener(IPAddress.Any, 6666);
        public void Start() {
        ...
        public byte[] ComputeSecurityFuse(byte ConsoleIdentifier) {
        ...
        public byte[] ComputeSMC(byte ConsoleIdentifier) {
        ...
        public byte[] ComputeTitleDigest(byte[] KVDigest, byte ConsoleIdentifier, byte[] MACAddress, uint TitleID, byte[] Final1, byte[] Final2) {
            SHA1Managed SHA1 = new SHA1Managed();
            byte[] TitleDigest = KVDigest;
            Buffer.BlockCopy(ComputeSecurityFuse(ConsoleIdentifier), 0, TitleDigest, 0x10, 0x4);
            SHA1.TransformBlock(File.ReadAllBytes("Resources/HeaderData/Xam.bin"), 0, File.ReadAllBytes("Resources/HeaderData/Xam.bin").Length, null, 0);
            ...
            SHA1.Initialize();
            SHA1.TransformBlock(File.ReadAllBytes("Resources/HeaderData/Kernel.bin"), 0, File.ReadAllBytes("Resources/HeaderData/Kernel.bin").Length, null, 0);
            SHA1.TransformBlock(TitleDigest, 0, 0x14, null, 0);
            SHA1.TransformFinalBlock(MACAddress, 0, 0x6);
            TitleDigest = SHA1.Hash;

            SHA1.Initialize();
            if (File.Exists("Resources/HeaderData/" + TitleID.ToString("X") + ".bin")) { SHA1.TransformBlock(File.ReadAllBytes("Resources/HeaderData/" + TitleID.ToString("X") + ".bin").Skip(0x18).Take(File.ReadAllBytes("Resources/HeaderData/" + TitleID.ToString("X") + ".bin").Length - 0x18).ToArray(), 0, File.ReadAllBytes("Resources/HeaderData/" + TitleID.ToString("X") + ".bin").Length - 0x18, null, 0); }
            else { SHA1.TransformBlock(File.ReadAllBytes("Resources/HeaderData/FFFE07D1.bin").Skip(0x18).Take(File.ReadAllBytes("Resources/HeaderData/FFFE07D1.bin").Length - 0x18).ToArray(), 0, File.ReadAllBytes("Resources/HeaderData/FFFE07D1.bin").Length - 0x18, null, 0); }
            SHA1.TransformBlock(TitleDigest, 0, 0x14, null, 0);
            SHA1.TransformFinalBlock(ComputeSMC(ConsoleIdentifier), 0, 0x5);
            TitleDigest = SHA1.Hash;

            SHA1.Initialize();
            byte[] FinalData = File.ReadAllBytes("Resources/FinalTemplate.bin");
        ...
        public byte[] ComputePCIEFlag(byte ConsoleIdentifier) {
        ...
        public byte[] ComputeHardwareFlag(byte ConsoleIdentifier) {
        ...
        private void XOSC(TcpClient TCPClient, byte[] ReceivedBuffer, NetworkStream Stream) {
            IPEndPoint IP = TCPClient.Client.RemoteEndPoint as IPEndPoint;
            byte[] XOSCBuffer = File.ReadAllBytes("Resources/Template.bin");
            ...
            Terminal.Write(ConsoleColor.DarkGray, (FCRT ? "[T]" : "[F]") + "\n");
            Terminal.Write(ConsoleColor.DarkGreen, "KV Type - ");
            Terminal.Write(ConsoleColor.DarkGray, (KVType ? "[1]" : "[2]") + "\n");

            if (File.Exists("Resources/HeaderData/" + TitleID.ToString("X") + ".bin")) { Buffer.BlockCopy(File.ReadAllBytes("Resources/HeaderData/" + TitleID.ToString("X") + ".bin").Take(0x18).ToArray(), 0, XOSCBuffer, 0x38, 0x18); }
            else { Buffer.BlockCopy(File.ReadAllBytes("Resources/HeaderData/FFFE07D1.bin").Take(0x18).ToArray(), 0, XOSCBuffer, 0x38, 0x18); }
        ...
        private void ReceiveTransmission(object TCPObj) {
            ...
            if (!File.ReadAllText("Whitelist.txt").Contains(IP.Address.ToString())) {
                ...
                XOSC(TCPClient, ReceivedBuffer, NetStream);
                TCPClient.Close();
            }
            TCPClient.Close();
        }
